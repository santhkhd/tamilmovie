...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const els = {
    ...
    backToYears: document.getElementById("backToYearsBtn"),
    yearMoviesTitle: document.getElementById("yearMoviesTitle"),
  };
  ...
  const renderMoviesForYear = (year) => {
    showMoviesView();
    els.yearMoviesTitle.textContent = `Movies from ${year} (${state.all.filter(m => m.year === year).length})`;
    els.moviesGrid.innerHTML = "";
    els.moviesGrid.appendChild(els.yearMoviesTitle);
  ...
  const showYearsView = () => {
    ...
    els.moviesGrid.style.display = "none";
    if (els.backToYears) {
      els.backToYears.style.display = "none";
    }
  };
  ...
  const showMoviesView = () => {
    ...
    if (els.backToYears) {
      els.backToYears.style.display = "inline-flex";
    }
  };
